						...
						>
							...
							<el-form-item prop="captcha">
								...
								>
									<el-button
										slot="append"
										@click="registerGetCaptcha"
										v-if="registerWait"
										>{{ registerWait }}</el-button
									>
									<el-button slot="append" v-if="!registerWait">{{
										`已发送 ${registerWaitTime}s`
									}}</el-button>
								</el-input>
							</el-form-item>
							<!-- 注册按钮 -->
		...
		registerGetCaptcha() {
			...
			this.$refs.registerForm.validateField('phone', async (error) => {
				if (!error) {
					...
					const { data: verifyPhone } = await this.$api.verifyPhone(phone);
					if (verifyPhone.exist === 1) {
						return this.$message.warning('该手机号已被注册');
					}
					// 发送验证码
					try {
						const { data: getCaptcha } = await this.$api.getCaptcha(phone);
			...
			switch (formName) {
				...
				case 'phoneForm':
					this.$refs.phoneForm.validate(async (valid) => {
						if (valid) {
							try {
								this.inLogin = true;
								const { data: res } = await this.$api.phoneLogin(
									this.phoneForm
								);
								// 登录失败
								if (res.code !== 200) {
				...
				case 'emailForm':
					this.$refs.emailForm.validate(async (valid) => {
						if (valid) {
							let email = this.emailForm.email + '@163.com';
							let password = this.emailForm.password;
							this.inLogin = true;
							const { data: res } = await this.$api.emailLogin(email, password);
							// 登录失败
							if (res.code !== 200) {
								this.inLogin = false;
								return this.$notify.error({
				...
				case 'registerForm':
					this.$refs.registerForm.validate(async (valid) => {
						let phone = this.registerForm.phone;
						let captcha = this.registerForm.captcha;
						if (valid) {
							// 开始验证验证码
							try {
								const { data: verify } = await this.$api.verifyCaptcha(
									phone,
									captcha
								);
								if (verify.code !== 200) {
							...
							// 开始注册
							const { data: res } = await this.$api.register(this.registerForm);
							if (res.code !== 200) {
