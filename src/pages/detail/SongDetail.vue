							...
							<div class="top-right">
								...
								<div class="top-right-userInfo">
									...
									<div class="userInfo-time">
										更新时间：{{ detail.updateTime | timeStamp }}
									</div>
								</div>
								<div class="top-right-tag">
									标签：
									<span v-if="(detail.tags || {}).length">
										<el-tag v-for="(item, index) in detail.tags" :key="index">{{
											item
										}}</el-tag>
									</span>
									<el-tag v-else>暂无标签</el-tag>
								</div>
								<div class="top-right-description" v-if="detail.description">
									{{ detail.description }}
								</div>
...
export default {
	...
	methods: {
		...
		// 加载歌单详情
		async loadSongSheetDetail() {
			const { data: res } = await this.$api.getSongSheetDetail(this.songListId);
			if (res.code !== 200) {
		...
		async loadSubscribers() {
			const { data: res } = await this.$api.getSubscribers(this.songListId);
			if (res.code !== 200) {
		...
		// 加载相关推荐
		async loadFeatured() {
			const { data: res } = await this.$api.getFeatured(this.songListId);
			if (res.code !== 200) {
		...
		// 加载精彩评论
		async loadComments() {
			const { data: res } = await this.$api.getMarvellousComments(
				this.songListId
			);
			if (res.code !== 200) {
			...
			// 如果没有精彩评论就给普通评论12条
			this.hotComments =
				res.hotComments.length === 0
					? res.comments.splice(0, 12)
					: res.hotComments;
		},
		// 加载歌曲详情
		async loadSongDetail(trackIds) {
			...
			const { data: res } = await this.$api.getSongDetail(trackId);
			if (res.code !== 200) {
		...
		async loadUserSong(cookie, userId) {
			if (!cookie) return;
			const { data: res } = await this.$api.getUserSong(cookie, userId);
			if (res.code !== 200) {
		...
		async favorite() {
			...
			const { data: res } = await this.$api.setFavorite(
				this.ifFavorite,
				this.songListId,
				cookie
			);
