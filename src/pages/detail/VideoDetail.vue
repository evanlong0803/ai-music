		...
		<el-row :gutter="20">
			...
			<el-col :span="17">
				...
				<el-card class="left-card">
					...
					<div class="left-time">
						...
						<span
							>播放次数：{{
								Number(videoDetail.playTime) ||
									Number(videoDetail.playCount) | playCount
							}}</span
						>
					</div>
					<!-- 点赞转发评论 -->
			...
			<el-col :span="7">
				...
				<el-card class="right-card" shadow="hover">
					...
					<div v-else-if="videoDetail.desc" class="right-description">
						{{ videoDetail.desc }}
					</div>
				...
				<el-card class="right-card" shadow="hover">
					...
					<div
						...
						v-for="(item, index) in relevantVideo"
						:key="index"
						@click="reload(item.vid || item.id)"
					>
						<el-image
							class="video-img"
...
export default {
	...
	methods: {
		...
		// 加载视频播放地址
		async loadVideoURL() {
			const { data: res } = await this.$api.getVideoURL(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
		...
		async loadVideoRelay() {
			const { data: res } = await this.$api.getVideoRelay(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
		...
		async loadVideoComment() {
			const { data: res } = await this.$api.getVideoComment(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
		...
		async loadRelevantVideo() {
			const { data: res } = await this.$api.getRelevantVideo(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
		...
		// 加载MV详情
		async loadMVDetail() {
			const { data: res } = await this.$api.getMVDetail(this.videoId);
			if (res.code !== 200) {
		...
		// 加载MV URL
		async loadMVURL() {
			const { data: res } = await this.$api.getMVURL(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
		...
		// 加载MV点赞转发评论数数据
		async loadMVRelay() {
			const { data: res } = await this.$api.getMVRelay(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
			this.$set(this.videoDetail, 'shareCount', res.shareCount);
		...
		// 加载MV评论
		async loadMVComment() {
			const { data: res } = await this.$api.getMVComment(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
			this.videoComment = res.hotComments.concat(res.comments);
		...
		async loadRelevantMV() {
			const { data: res } = await this.$api.getRelevantMV(this.videoId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
