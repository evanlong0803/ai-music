	...
	<div class="detail">
		...
		<div class="detail-top">
			...
			<div class="top-avatar">
				...
				<div>
					<i
						v-if="userDetail.level"
						:class="`iconfont icon-icon-test${userDetail.level}`"
					></i>
					{{ singerInfo.name }}
					<i
						:class="{
			...
			<img
				class="top-modification"
				src="../../assets/image/arrow-lr.5aa8d800.png"
			/>
		</div>
...
export default {
	...
	methods: {
		...
		async loadUserDetail(cookie) {
			const uid = this.$route.query.accountId;
			const { data: res } = await this.$api.getUserDetail(uid, cookie);
			if (res.code !== 200) {
				return this.$message.error('用户详情请求失败');
			}
			this.userDetail = res;
		},
		// 加载歌手单曲
		async loadSingerSingle() {
			const { data: res } = await this.$api.getSingerSingle();
			if (res.code !== 200) {
				return this.$message.error('歌手单曲请求失败');
			}
		...
		async loadSingerAlbum() {
			const { data: res } = await this.$api.getSingerAlbum(this.singerId);
			if (res.code !== 200) {
		...
		// 加载歌手MV
		async loadSingerMV() {
			const { data: res } = await this.$api.getSingerMV(this.singerId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
			this.video = res.mvs;
		...
		async loadSingerProfile() {
			const { data: res } = await this.$api.getSingerProfile(this.singerId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
			this.profile = res;
		...
		async loadSingerSimilar() {
			const { data: res } = await this.$api.getSingerSimilar(this.singerId);
			if (res.code !== 200) {
				return this.$message.error('请求失败');
			}
