				...
				<el-card class="personal-right">
					...
					<div class="right-bottom">
						...
						<ul class="bottom-info">
							...
							<li>
								生日： {{ (userDetail.profile || {}).birthday | timeStampTwo }}
							</li>
							<!-- 地区：{{ (userDetail.profile || {}).province }} {{ (userDetail.profile || {}).city }} -->
							<li>
								地区：{{ (regionProvince || {}).region }} {{ regionCity }}
							</li>
						</ul>
						...
						<div class="bottom-button" v-if="independent">
							<el-button type="danger" size="medium" @click="goSetting(userId)"
								>个人设置</el-button
							>
							<el-button type="danger" size="medium" @click="goGrade"
								>我的等级</el-button
							>
						</div>
					</div>
...
export default {
	...
	methods: {
		...
		async loadUserDetail(cookie) {
			const { data: res } = await this.$api.getUserDetail(this.userId, cookie);
			if (res.code !== 200) {
		...
		async goSignIn() {
			let cookie = localStorage.getItem('cookie');
			const { data: res } = await this.$api.signIn(cookie);
			if (res.code !== 200) {
...
.personal {
	...
	.personal-bottom {
		...
		.personal-right {
			...
			.right-top {
				height: 150px;
				background: url('https://p1.music.126.net/_f8R60U9mZ42sSNvdPn2sQ==/109951162868126486.jpg')
					no-repeat;
				margin: -20px -20px 0;
			}
			.right-bottom {
